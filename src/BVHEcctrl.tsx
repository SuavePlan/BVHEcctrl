...
const BVHEcctrl = forwardRef<BVHEcctrlApi, EcctrlProps>(
  (
     ...
     */
    useEffect(() => {
      const unsubscribeJoystick = useJoystickStore.subscribe(({ joystickX, joystickY }) =>
    ...
    const localMinDistance = useRef<number>(Number.POSITIVE_INFINITY);
    ...
    const globalMinDistance = useRef<number>(Number.POSITIVE_INFINITY);
    ...
    const floatRaycastCandidates = useMemo(
      () => colliderMeshesArray.filter((mesh) => mesh.geometry.boundsTree && !(mesh instanceof THREE.InstancedMesh)),
      [colliderMeshesArray]
    );
      ...
      (mesh: THREE.Mesh, originMatrix: THREE.Matrix4, delta: number) => {
        ...
        if (triangleCount.current > 0) {
          ...
          if (isStatic) {
            ...
            platformVelocityAtContactPoint.current
              .copy(mesh.userData.linearVelocity)
              .add(contactPointRotationalVel.current);
    ...
    const floatingCheck = useCallback(
      (mesh: THREE.Mesh, originMatrix: THREE.Matrix4) => {
        ...
        localClosestPoint.current.set(Number.POSITIVE_INFINITY, Number.POSITIVE_INFINITY, Number.POSITIVE_INFINITY);
        ...
        mesh.geometry.boundsTree?.shapecast({
          ...
          intersectsTriangle: (tri) => {
            ...
            const verticalLength =
              Math.abs(dot) / ((capsuleRadius + floatHeight + floatPullBackHeight) / floatSensorRadius);
    ...
    const handleFloatingResponse = useCallback(
      (colliderMeshesArray: THREE.Mesh[], jump: boolean, delta: number) => {
        ...
        globalClosestPoint.current.set(Number.POSITIVE_INFINITY, Number.POSITIVE_INFINITY, Number.POSITIVE_INFINITY);
        ...
        if (globalMinDistance.current < Number.POSITIVE_INFINITY) {
